...
class Comment {
    ...
    public function create(int $uid, int $post_id, string $text) {
        $stmt = Db::getInstance()->getConnection()->prepare("INSERT INTO comments(`user_id`, `post_id`, `comment`) VALUES(:user_id, :post_id, :comment)");
    ...
    public function allPosts() {
        $stmt = Db::getInstance()->getConnection()->prepare("SELECT comments.*, users.username FROM `comments` INNER JOIN `users` ON comments.user_id = users.id ORDER BY id DESC;");
        $stmt->execute();
        $comments = [];
    ...
    public function deleteComment($id) {
        $stmt = Db::getInstance()->getConnection()->prepare("DELETE FROM comments WHERE `id` = :id;");
        $stmt->execute(['id' => $id]);
    }
    ...
    public static function getDate($id) {
        $stmt = Db::getInstance()->getConnection()->prepare("SELECT comments.date FROM `comments` WHERE `id` = :id;");
        $stmt->execute(['id' => $id]);
    ...
    public function editComment($id, $comment) {
        $stmt = Db::getInstance()->getConnection()->prepare("UPDATE `comments` SET `comment` = :comment WHERE `id` = :id;");
